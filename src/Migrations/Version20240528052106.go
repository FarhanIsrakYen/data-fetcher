...
func CreateReliableHyperTable(db *gorm.DB) {

	var tableName = "df_data_reliables"
	var exists bool
	...
	if !exists {
		if err := db.Exec("SELECT create_hypertable('" + tableName + "', 'time', chunk_time_interval => INTERVAL '30 day');").Error; err != nil {
			panic(err.Error())
			return
		}
